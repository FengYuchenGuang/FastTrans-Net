...
class ShuffleNet_old(nn.Module):
...
class ShuffleNet(nn.Module):
    """ShuffleNet for groups=4"""
    def __init__(self, groups=4, in_channels=3, num_classes=2048):
        super(ShuffleNet, self).__init__()

        self.group = groups
        self.conv1 = nn.Conv2d(in_channels, 64, 3, stride=2, padding=1)
        stage2_seq = [ShuffleNetUnitB(64, 512, groups=self.group)] + \
            [ShuffleNetUnitA(512, 512, groups=self.group) for i in range(3)]
        self.stage2 = nn.Sequential(*stage2_seq)
        stage3_seq = [ShuffleNetUnitB(512, 1024, groups=self.group)] + \
            [ShuffleNetUnitA(1024, 1024, groups=self.group) for i in range(7)]
        self.stage3 = nn.Sequential(*stage3_seq)
        stage4_seq = [ShuffleNetUnitB(1024, 2048, groups=self.group)] + \
                     [ShuffleNetUnitA(2048, 2048, groups=self.group) for i in range(3)]
        self.stage4 = nn.Sequential(*stage4_seq)
        self.fc = nn.Linear(960, num_classes)

    def forward(self, x):
        # print("ShuffleNet_in", x.shape)
        net = self.conv1(x)
        net = F.max_pool2d(net, 3, stride=1, padding=1)
        net = self.stage2(net)
        net = self.stage3(net)
        net = self.stage4(net)

        logits = net
        # print("ShuffleNet_F.softmax", logits.shape)
        return logits
