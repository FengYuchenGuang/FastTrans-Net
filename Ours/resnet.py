...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers, multi_scale=False):
        ...
        if num_layers == 50:
            ...
            resnet.load_state_dict(
                torch.load(
                    "./Ours/TrainModule/resnet50-19c8e357.pth"
                ))
            # remove fully connected layer, avg pool and layer5:
            if self.multi_scale:
                self.c2_layers = nn.Sequential(*list(resnet.children())[:5])
                self.c3_layers = nn.Sequential(*list(resnet.children())[5:6])
                self.c4_layers = nn.Sequential(*list(resnet.children())[6:-3])
            else:
            ...
            resnet.load_state_dict(
                torch.load(
                    "./Ours/TrainModule/resnet101-5d3b4d8f.pth"
                ))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            resnet.load_state_dict(
                torch.load(
                    "./Ours/TrainModule/resnet152-b121ed2d.pth"
                ))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
    ...
    def forward(self, x):
        ...
        if self.multi_scale:
            c2 = self.c2_layers(x)  # (shape: (batch_size, 256, h/4 , w/4))
            c3 = self.c3_layers(c2)  # (shape: (batch_size, 2*256, h/8 , w/8))
            c4 = self.c4_layers(c3)
        else:
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers, multi_scale=False):
        ...
        if num_layers == 18:
            ...
            resnet.load_state_dict(
                torch.load(
                    "./Ours/TrainModule/resnet18-5c106cde.pth")
            )
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            resnet.load_state_dict(
                torch.load(
                    "./Ours/TrainModule/resnet34-333f7ec4.pth")
            )
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
def ResNet18_OS16(multi_scale=False):
    return ResNet_BasicBlock_OS16(num_layers=18, multi_scale=multi_scale)
...
def ResNet50_OS16(multi_scale=False):
    return ResNet_Bottleneck_OS16(num_layers=50, multi_scale=multi_scale)
